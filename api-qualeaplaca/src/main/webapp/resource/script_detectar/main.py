...
def clean_images():
	file_list = os.listdir(dir_path+"\outputs\images")
	for file_name in file_list:
		if '.png' in file_name:
			os.remove(dir_path+"\outputs\images" + file_name)
...
def binarization(image):
    thresh = cv2.threshold(image,0,50,cv2.THRESH_BINARY)[1]
...
def localization(image, min_size_components, similitary_contour_with_circle, model, count, current_sign_type):
    ...
    if sign is not None:
        ...
        if os.path.exists(dir_path+"\outputs\images\\" + text + '.png') == False and text != "ERROR":
            cv2.imwrite(dir_path+"\outputs\images\\" + text + '.png', sign)
...
def main(args):
    ...
    out = cv2.VideoWriter(dir_path+"\outputs\output.avi",fourcc, fps , (640,480))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
      '--file_name',
      default= dir_path+"\inputs\input.avi",
